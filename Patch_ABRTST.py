...
from Patch_ABRTST_layers import *
from Patch_ABRTST_layers import RevIN
from Resnet_CBAM import ChannelAttention


# Cell
class PatchTST_backbone(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int, n_layers:int,d_model:int,n_heads:int,
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        z = self.globalmaxpool(z)  # (bs,patch_num,1)
        z = z.contiguous().view(-1, z.shape[1] * z.shape[2])
        z = self.fc(z)
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, patch_num, patch_len, max_seq_len=1024,
                 n_layers=5, d_model=128, n_heads=16, d_k=None, d_v=None,
